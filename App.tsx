...
const NextButton = () => {
  ...
  return (
    <button
      ...
      } flex flex-col items-center gap-2 text-white animate-bounce cursor-pointer z-10 hover:text-cyan-300 transition-colors`}
    >
      <span className="text-sm font-medium tracking-widest uppercase">
        {isVertical ? "Scroll Down" : "Scroll Right"}
      </span>
      <svg
        width="24"
        height="24"
...
// This remains inside FullPage, but thanks to the fix, it is rendered outside the transform container
const Header = ({
  onToggleDirection,
  direction,
}: {
...
const App: React.FC = () => {
  ...
  const fullPageRef = useRef<FullPageRef>(null); // External control ref
  ...
  return (
    <>
      {/* External Control Example: Allows controlling the page from completely outside the FullPage tree */}
      <div className="fixed bottom-4 left-4 z-[100] flex flex-col gap-2">
        <button
          onClick={() => fullPageRef.current?.goTo(0)}
          className="bg-pink-600 text-white text-xs px-3 py-2 rounded shadow-lg hover:bg-pink-700 transition"
        >
          External Reset (via Ref)
        </button>
      </div>

      {/* FullPage Wrapper with Ref attached */}
      <FullPage
        ref={fullPageRef}
        onLeave={handleLeave}
        duration={800}
        direction={direction}
      >
        {/* Helper Components (now rendered as overlays, so they stay fixed!) */}
        <NavigationDots />
        ...
        <Section className="bg-gradient-to-br from-slate-900 to-slate-800 text-white flex items-center justify-center relative">
          <div className="text-center max-w-2xl px-6">
            ...
            <p className="text-lg md:text-xl text-slate-300 mb-8 leading-relaxed">
              A lightweight, high-performance React library for fullscreen
              scrolling. Try dragging, scrolling, or using the arrow keys.
            </p>
            <div className="flex justify-center gap-4">
              <button
                onClick={toggleDirection}
                className="px-8 py-3 bg-cyan-500 hover:bg-cyan-600 text-white font-bold rounded-lg transition-all transform hover:scale-105 shadow-lg shadow-cyan-500/25"
              >
                Switch to {direction === "vertical" ? "Horizontal" : "Vertical"}
              </button>
              <button
                onClick={() => fullPageRef.current?.next()}
                className="px-8 py-3 bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/10 text-white font-bold rounded-lg transition-all"
              >
                Next (Ref)
              </button>
            </div>
        ...
        <Section className="bg-white text-slate-900 flex items-center justify-center relative">
          ...
          <div className="max-w-6xl w-full px-6 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <div>
              <h3 className="text-sm font-bold text-blue-600 tracking-widest uppercase mb-2">
                Features
              </h3>
              ...
              <p className="text-lg text-slate-600 mb-6">
                Built with modern React hooks and Tailwind CSS. It handles mouse
                wheel, touch swipes, and drag gestures seamlessly across all
                devices.
              </p>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-50 p-6 rounded-2xl shadow-xl transform translate-y-8">
                ...
                <p className="text-sm text-slate-500">
                  Optimized for 60fps animations with CSS transforms.
                </p>
              </div>
          ...
          <div className="w-full h-full relative">
            ...
            <div className="relative z-10 flex flex-col items-center justify-center h-full text-center px-4">
              ...
              <p className="mt-8 text-slate-400 max-w-lg">
                We expose a simple API hook <code>useFullPage()</code> allowing
                you to build custom controls anywhere inside the provider.
              </p>
            </div>
          </div>
          <NextButton />
            ...
            <form className="bg-white p-8 rounded-2xl shadow-2xl text-left space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none text-slate-800"
                />
              </div>
              <button
                type="button"
